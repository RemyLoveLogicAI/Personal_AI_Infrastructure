...
const isHighBlastRadiusAction = gatesModule.isHighBlastRadiusAction
...
describe("gates bus", () => {
 ...
 test("blast radius detects rm -rf and git push --force", async () => {
  const resRm = isHighBlastRadiusAction("bash", { command: "rm -rf /tmp/data" })
  expect(resRm.isHighBlast).toBe(true)
  expect(resRm.reason).toContain("rm -rf")

  const resPush = isHighBlastRadiusAction("bash", { command: "git push origin main --force" })
  expect(resPush.isHighBlast).toBe(true)
  expect(resPush.reason).toContain("git push --force")

  const resSafe = isHighBlastRadiusAction("bash", { command: "ls -la" })
  expect(resSafe.isHighBlast).toBe(false)
 })

 test("blast radius detects PAI identity modification", async () => {
  const resIdentity = isHighBlastRadiusAction("write", { path: "/Users/user/.claude/PAI/TELOS.md" })
  expect(resIdentity.isHighBlast).toBe(true)

  const resSafe = isHighBlastRadiusAction("write", { path: "src/safe.ts" })
  expect(resSafe.isHighBlast).toBe(false)
 })

 test("POST /api/gates/blast_radius evaluates destructive actions", async () => {
  const res = await post("/api/gates/blast_radius", {
   toolName: "bash",
   toolInput: { command: "dd if=/dev/zero of=/dev/sda" }
  })
  expect(res.status).toBe(200)
  const body = await res.json()
  expect(body.isHighBlast).toBe(true)
  expect(body.reason).toContain("dd if=")
 })
})
