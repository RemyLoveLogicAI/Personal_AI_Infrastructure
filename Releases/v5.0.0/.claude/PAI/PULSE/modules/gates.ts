...
/**
 * Blast-radius evaluation rule for destructive execution & identity modifications.
 */
export function isHighBlastRadiusAction(toolName: string, input: unknown): { isHighBlast: boolean; reason?: string } {
 const normTool = (toolName || "").toLowerCase().trim()

 if (normTool === "bash" || normTool === "exec" || normTool === "terminal") {
  let cmd = ""
  if (typeof input === "string") cmd = input
  else if (input && typeof input === "object") {
   const obj = input as Record<string, unknown>
   cmd = String(obj.command ?? obj.cmd ?? "")
  }

  if (/\brm\s+(-[a-zA-Z]*r[a-zA-Z]*f|-[a-zA-Z]*f[a-zA-Z]*r)\b/i.test(cmd)) {
   return { isHighBlast: true, reason: "Destructive file removal (`rm -rf`)" }
  }
  if (/\bgit\s+push\b.*(--force|-f\b|\+)/i.test(cmd)) {
   return { isHighBlast: true, reason: "Force pushing git repository (`git push --force`)" }
  }
  if (/\bdd\s+if=/i.test(cmd)) {
   return { isHighBlast: true, reason: "Direct disk overwrite command (`dd if=`)" }
  }
  if (/\bmkfs\b/i.test(cmd)) {
   return { isHighBlast: true, reason: "Filesystem format command (`mkfs`)" }
  }
 }

 if (normTool === "write" || normTool === "edit" || normTool === "multiedit") {
  let targetPath = ""
  if (typeof input === "string") targetPath = input
  else if (input && typeof input === "object") {
   const obj = input as Record<string, unknown>
   targetPath = String(obj.path ?? obj.file_path ?? obj.target ?? "")
  }

  if (
   targetPath.includes(".claude/PAI/") ||
   targetPath.includes(".claude/PAI") ||
   targetPath.includes("PAI/USER") ||
   targetPath.includes("TELOS.md")
  ) {
   return { isHighBlast: true, reason: `Modification of critical PAI identity/telos file: ${targetPath}` }
  }
 }

 return { isHighBlast: false }
}
...
export async function handleGatesRequest(req: Request, pathname: string): Promise<Response | null> {
 ...
 // POST /api/gates/blast_radius — evaluate blast radius for destructive actions
 if (req.method === "POST" && pathname === "/api/gates/blast_radius") {
  const toolName = str(body.toolName) ?? ""
  const toolInput = body.toolInput ?? body.input ?? {}
  const evalResult = isHighBlastRadiusAction(toolName, toolInput)
